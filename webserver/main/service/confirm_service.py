...
def send_confirm_payload_to_client(message):
    ...
    confirm_payload = get_first_ondc_request(
        OndcDomain.RETAIL, OndcAction('confirm'), confirm_message_id)
    resp, return_code = make_retail_confirm_payload_request_to_client(
        confirm_payload)
...
def send_confirm_response_to_bap(message):
    ...
    on_confirm_payload = get_first_ondc_request(
        OndcDomain.RETAIL, OndcAction('on_confirm'), on_confirm_message_id)
    bap_endpoint = on_confirm_payload['context']['bap_uri']
    status_code = make_request_over_ondc_network(
        on_confirm_payload, bap_endpoint, 'on_confirm')
